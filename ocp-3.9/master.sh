...
# Deploy template service broker pods on master region
...
# Configuring Ansible Service Broker storage and dev mode
...
openshift_metrics_public_url=https://metrics.${ROUTEREXTIP}.nip.io/hawkular/metrics
...
- hosts: all
  tasks:
  - name: ensure local azure domain is present
    lineinfile:
      path: /etc/resolv.conf
      regexp: '^search'
      line: 'search ${SUBNET_REF} cluster.local'
EOF
